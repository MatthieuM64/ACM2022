...
class particle
{
	...
	particle(const int &init, const int &LX, const int &LY, const double &theta0);
	void hop(const bool &favdir, const int &LX, const int &LY);
	void flip(const double &thetap);
};

//Creation of the spin.
particle::particle(const int &init, const int &LX, const int &LY, const double &theta0)
{
...
void particle::hop(const bool &favdir, const int &LX, const int &LY)
{
	double phi;
...
class averages
{
	...
	void update(const int &Npart, const vector<particle> &AXYP, const int &LX, const int &LY);
	void exportFile(const double &beta, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN);
};
...
void averages::update(const int &Npart, const vector<particle> &AXYP, const int &LX, const int &LY)
{
	...
	{
		RHO[int(AXYP[i].x)][int(AXYP[i].y)]++;
		MX[int(AXYP[i].x)][int(AXYP[i].y)]+=cos(AXYP[i].theta);
		MY[int(AXYP[i].x)][int(AXYP[i].y)]+=sin(AXYP[i].theta);
	}
...
void averages::exportFile(const double &beta, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN)
{
	static const int returnSystem=system("mkdir -p data_AXYM_averages/");
	stringstream ss;	
	ss << "./data_AXYM_averages/AXYM_fluctuations_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";	
...
vector<double> mag(const int &Npart, const vector<particle> &AXYP)
{
	double MX=0, MY=0;
	for (int i=0; i<Npart; i++)
	{
...
vector<double> msd(const int &Npart, const vector<particle> &AXYP)
{
	double DX=0, DY=0;
	double DX2=0., DY2=0.;
	for (int i=0; i<Npart; i++)
	{
		DX+=AXYP[i].dx/Npart;
		DX2+=AXYP[i].dx*AXYP[i].dx/Npart;
		DY+=AXYP[i].dy/Npart;
		DY2+=AXYP[i].dy*AXYP[i].dy/Npart;
	}	
...
void exportDensity(const double &beta, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t, const int &Npart, const vector<particle> &AXYP)
{
	vector< vector<int> > RHO(LX,vector<int>(LY,0));
	for (int i=0; i<Npart; i++)
	{
	...
	static const int returnSystem=system("mkdir -p data_AXYM_dynamics/");
	stringstream ssRHO;
	
	ssRHO << "./data_AXYM_dynamics/AXYM_RHO_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";	
...
void exportMagnetization(const double &beta, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t, const int &Npart, const vector<particle> &AXYP)
{
	vector< vector<double> > MX(LX,vector<double>(LY,0.)), MY(LX,vector<double>(LY,0.));
	for (int i=0; i<Npart; i++)
	{
	...
	ssMX << "./data_AXYM_dynamics/AXYM_MX_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";	
	string nameMX = ssMX.str();
	ofstream fileMX(nameMX.c_str(),ios::trunc);
	fileMX.precision(6);
	
	ssMY << "./data_AXYM_dynamics/AXYM_MY_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";	
...
void ReadCommandLine(int argc, char** argv, double &beta, double &epsilon, double &rho0, int &LX, int &LY, int& init, int &RAN, int &tmax)
{
...
{
	//Physical parameters: beta=inverse temperature, epsilon=self-propulsion, rho0=average density, LX*LY=size of the box.
	...
	ReadCommandLine(argc,argv,beta,epsilon,rho0,LX,LY,init,RAN,tmax);
	...
	//Creation of active XY particles.
	vector<particle> AXYP(0,particle(init,LX,LY,0));
	const double theta0=2*M_PI*ran();
	for (int i=0;i<Npart;i++)
	{
		particle AXYP0(init,LX,LY,theta0);
		SEC.add(int(AXYP0.x),int(AXYP0.y),i);
		AXYP.push_back(AXYP0);
	}
	
	//Creation of file for averages.
	const int returnSystem=system("mkdir -p data_AXYM_averages/");
	stringstream strAVERAGES;
	strAVERAGES << "./data_AXYM_averages/AXYM_AVERAGES_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		if (t>teq)
		{
			AV.update(Npart,AXYP,LX,LY);
		}
		if (t%(tmax/100)==0 or t==tmax)
		{
			exportDensity(beta,epsilon,rho0,LX,LY,init,RAN,t,Npart,AXYP);
			exportMagnetization(beta,epsilon,rho0,LX,LY,init,RAN,t,Npart,AXYP);
			AV.exportFile(beta,epsilon,rho0,LX,LY,init,RAN);
		}
		...
		for (int i=0;i<Npart;i++)
		{
			...
			const int X0=int(AXYP[j].x), Y0=int(AXYP[j].y);
			const double theta=AXYP[j].theta;
			
			//Determination of the new orientation (uniformly).
			double thetap=2*M_PI*ran();
			...
			for (int XN=X0-1;XN<=X0+1;XN++)
			{
				for (int YN=Y0-1;YN<=Y0+1;YN++)
				{
					...
					for (int ll=0; ll<neighbours.size(); ll++)
					{
						...
						if (j!=k and distance2(AXYP[j],AXYP[k],LX,LY)<1)
						{
							MX+=cos(AXYP[k].theta);
							MY+=cos(AXYP[k].theta);
							rhoj++;
			...
			{
				AXYP[j].hop(random_number<proba_hop*epsilon,LX,LY);
				
				//UPDATE OF SECTOR!
				if (X0!=int(AXYP[j].x) or Y0!=int(AXYP[j].y))
				{
					SEC.remove(X0,Y0,j);
					SEC.add(int(AXYP[j].x),int(AXYP[j].y),j);
				}
			}
